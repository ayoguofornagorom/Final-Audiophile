...
export const login = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    //Check user exists AND paswword matches
    //user mmatchPassword is defined on the user model
    ...
    if (user && (await user.matchPassword(password))) {
      ...
      // (don't tell hackers whether the email or password is wrong)
...
export const getProfile = async (
  ...
  try {
    // reg.user is set by the "protect" middleware
...
export const updateAvatar = async (
  ...
  try {
    ...
    if (!req.file) {
      res.status(400).json({ messgae: "No image file provided" });
      return;
    }
    ...
    // save the new avatar url and public_id to the user's profile in MongoDB
...
//permanetly remove a user (admin only)
